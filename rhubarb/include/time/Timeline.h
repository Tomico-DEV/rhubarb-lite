...
public:
    OptionalRef() = default;
    OptionalRef(std::nullopt_t) : ref_(std::nullopt) {}
    OptionalRef(T& value) : ref_(std::ref(value)) {}
    OptionalRef(std::reference_wrapper<T> ref) : ref_(ref) {}

    bool has_value() const noexcept { return ref_.has_value(); }
    explicit operator bool() const noexcept { return ref_.has_value(); }

    T& value() { return ref_.value().get(); }
    const T& value() const { return ref_.value().get(); }
...
public:
    ...
    public:
        ...
        operator std::optional<T>() const {
            ...
            return optional ? std::make_optional(optional->getValue())
                            : std::nullopt;
        }

        // convert to const T& (keeps previous behavior)
        ...
        ReferenceWrapper& operator=(OptionalRef<const T> value) {
            if (value) {
    ...
    reverse_iterator rbegin() const { return elements.rbegin(); }
    reverse_iterator rend() const { return elements.rend(); }
    ...
    OptionalRef<const Timed<T>> get(time_type time) const {
        iterator it = find(time);
        return (it != end()) ? OptionalRef<const Timed<T>>(*it) : std::nullopt;
    }
    ...
    bool operator==(const Timeline& rhs) const { return equals(rhs); }
    bool operator!=(const Timeline& rhs) const { return !equals(rhs); }

protected:
    bool equals(const Timeline& rhs) const { return elements == rhs.elements; }
